...
def add_missing_orgunits(
    ...
    dataset_units = dhis2.api.get(
        endpoint=f"dataSets/{dataset_uid}?fields=organisationUnits[id]",
    )
...
def extract_data_from_esigl(
    ...
    products_code += f" AND rli.productcode IN {tuple(products_code) if len(products_code) > 1 else f'({products_code[0]!r})'}"  # noqa: E501
