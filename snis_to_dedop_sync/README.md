# SNIS → DEDOP — Synchronisation des dataValues

Pipeline OpenHEXA synchronisant les `dataValues` du **SNIS** vers **DEDOP** à partir des datasets.
Voir [`PLAN_AMELIORATION.md`](./PLAN_AMELIORATION.md) pour le détail des choix de conception.

## Hypothèses

- Les **UID** des `dataSets` et `dataElements` sont **identiques** entre SNIS et DEDOP (pas de mapping).
- L'**existence des organisation units** dans DEDOP est assurée en amont par le pipeline
  `snis_to_nmdr_sync_orgunits`, planifié **quotidiennement avant** cette synchro.
...
check santé + validation AOC cible
  └─ pour chaque dataset :
       ...
       détection COC manquants dans DEDOP → création optionnelle (consentement)
       extraction SNIS (racine + enfants, includeDeleted) → post-filtre orgUnits + COC valides
...
|-----------|------|--------|-------------|
| `snis_connection` / `dedop_connection` | DHIS2Connection | `snis-dhis2` / `dhis2-nmdr-temp` | Connexions source / cible |
...
| `dedop_target_aoc` | str | `HllvX50cXC0` | **AOC cible (DEDOP)** appliqué à toutes les valeurs |
| `create_missing_metadata` | bool | `False` | Créer les COC manquants dans DEDOP (UID préservés) — sinon ignorés + reportés |
...
| `use_cache` | bool | `False` | Cache des réponses API SNIS |
...
- **Suppressions** : extraites via `includeDeleted=true` et propagées à DEDOP (payload `deleted`).
