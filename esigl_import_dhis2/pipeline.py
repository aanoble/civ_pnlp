...
@parameter(
    ...
    help="File path for coc mapping cols located in directory `metadabase eSIGL/data/ressources`",
    default="metadabase eSIGL/data/ressources/mapping_coc.json",
    required=True,
)
@parameter(
    ...
    help="File path for district mapping cols located in directory `metadabase eSIGL/data/ressources`",  # noqa: E501
    default="metadabase eSIGL/data/ressources/mapping_district.json",
    required=True,
)
@parameter(
    "output_directory",
    ...
    default="metadabase eSIGL/data/output",
    required=True,
)
@parameter(
    "dhis2_aoc",
...
@parameter(
    "nb_months_to_refresh",
    type=int,
    name="Number of months to refresh",
    help="Number of months to refresh",
    default=3,
    choices=[1, 3, 6, 12],
    required=True,
)
    ...
    df_etat_stock = pl.DataFrame(
        mb_client.get_data_from_sql_query(
            QUERY_ETAT_STOCK.format(
                products_code=tuple(df_products["code"].unique().to_list()), period=months
            )
        )
    )
